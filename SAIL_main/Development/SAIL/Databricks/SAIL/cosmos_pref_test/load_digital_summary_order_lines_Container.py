...
cosmosDatabaseName = "SAIL_PREF_TEST"
...
def get_pre_cosmos_query():
  ...
  ,(select case when collect_set(named_struct('batchNumber',LD2.VendorLotNumber,'serialNumber',LD2.VendorSerialNumber,'expirationDate',LD2.EXPIRATION_DATE)) = array() then array(named_struct('batchNumber',null,'serialNumber',null,'expirationDate',null)) 
else collect_set(named_struct('batchNumber',LD2.VendorLotNumber,'serialNumber',LD2.VendorSerialNumber,'expirationDate',LD2.EXPIRATION_DATE))
end val
  from {digital_summary_order_lines_details} LD2 
  where 
  LD2.VendorLotNumber is not null AND
  LD2.UPSOrderNumber = OL.UPSOrderNumber AND 
  LD2.SourceSystemKey = OL.SourceSystemKey AND 
  LD2.itemNumber = OL.SKU AND
  LD2.LineNumber = OL.LineNUmber) as OrderLineDetail
  ,(select named_struct('VSN',collect_set(LD.VendorSerialNumber),
    'VCL',collect_set(LD.VendorLotNumber),
    'LPN',collect_set(LD.LPNNumber),
    'Designator',collect_set(LD.DispositionValue))
  ...
  LD.LineNumber = OL.LineNUmber) as OrderLineVendorDetail  
  ,OL.CarrierCode
  ...
  where is_inbound = 0
  and AccountId = '1EEF1B1A-A415-43F3-88C5-2D5EBC503529'
...
SELECT 
    ...
    (select case when 	collect_set(named_struct('batchNumber',LD.VendorLotNumber,'serialNumber',LD.VendorSerialNumber,'expirationDate',LD.EXPIRATION_DATE)) = array() then array(named_struct('batchNumber',null,'serialNumber',null,'expirationDate',null)) 
else collect_set(named_struct('batchNumber',LD.VendorLotNumber,'serialNumber',LD.VendorSerialNumber,'expirationDate',LD.EXPIRATION_DATE))
end val
    from {digital_summary_order_lines_details} LD 
    where 
