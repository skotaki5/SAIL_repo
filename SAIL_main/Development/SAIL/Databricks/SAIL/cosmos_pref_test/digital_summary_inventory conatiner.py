...
df=df.withColumn("AccountId",concat(df.AccountId,lit('_1')))
...
df=df.withColumn("AccountId",concat(df.AccountId,lit('_2')))
df=df.withColumn("id",concat(df.id,lit('_2')))
df.write.format("cosmos.oltp").options(**cfg2).mode("APPEND").save()

# COMMAND ----------

df=df.withColumn("AccountId",concat(df.AccountId,lit('_3')))
